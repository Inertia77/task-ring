...
const ROLLOVER_HOUR=4;function getOperationalDate(date){const d=new Date(date);if(d.getHours()<ROLLOVER_HOUR){d.setDate(d.getDate()-1)}return d}const realNow=new Date();const operationalNow=getOperationalDate(realNow);const today=operationalNow.getDay();const UI_VIEW_MODE_KEY="taskring_view_mode_v1";const UI_MOBILE_DAY_KEY="taskring_mobile_day_v1";const savedViewMode=localStorage.getItem(UI_VIEW_MODE_KEY);const savedMobileDay=Number(localStorage.getItem(UI_MOBILE_DAY_KEY));let viewMode=["undone","today","all"].includes(savedViewMode)?savedViewMode:"undone";let mobileDay=[0,1,2,3,4,5,6].includes(savedMobileDay)?savedMobileDay:today;
...
const UI_SCROLL_SELECTORS=[".viewDock",".weeklyCategoryTabs",".gameQuestModeTabs",".gameQuestFilterTabs",".timeSubTabs",".dayTabs",".taskEditorList",".refEditorList",".gameQuestEditorList"];
...
function openGameQuestTimeDetail(){
  ...
  const lines=week.slice(0,14).map(log=>`<li><span>${fmtLogWhen(log)} · ${escapeHtml(log.title)}</span><b>${fmtMinutes(log.duration_minutes)}</b><button type="button" data-time-log-delete="${escapeHtml(log.id)}">删除</button></li>`).join("")||`<li class="empty"><span>本周还没有游戏作战区记录</span><b>0m</b></li>`;
  const body=`<div class="timeDetailStats"><div><span>今日</span><b>${fmtMinutes(todayUsed)}</b></div><div><span>本周</span><b>${fmtMinutes(used)}</b></div><div><span>累计</span><b>${fmtMinutes(allUsed)}</b></div></div><div class="timeDetailProgress" style="--w:${Math.min(100,used?100:0)}%"><div><span>统计方式</span><b>整体计时汇总${activeMinutes?` · 含当前进行中 ${fmtMinutes(activeMinutes)}`:""}</b></div><i></i></div><ul class="timeDetailLogs">${lines}</ul>`;
...
function completeActiveTimer(markDone=true){
  ...
  if(markDone&&timer.kind==="task"){
    ...
    if(task&&!isWeeklyPoolTask(task)&&!isDone(task.id,Number(timer.day_id),timer.cycle||cycleYmd))setDone(task.id,Number(timer.day_id),true,null,true,timer.cycle||cycleYmd);
  }
...
function renderTimePanel(){
  ...
  const activeGameQuestMinutes=active&&active.kind==="gamequest"?Math.max(1,Math.round(activeTimerElapsedSeconds(active)/60)):0;
  const gameQuestWeekUsed=readTimeLogs().filter(log=>(log.kind==="gamequest"||log.task_id==="gamequest-board")&&isLogInCurrentCycle(log)).reduce((sum,log)=>sum+Number(log.duration_minutes||0),0)+activeGameQuestMinutes;
  const gameQuestTodayUsed=readTimeLogs().filter(log=>(log.kind==="gamequest"||log.task_id==="gamequest-board")&&isLogToday(log)).reduce((sum,log)=>sum+Number(log.duration_minutes||0),0)+activeGameQuestMinutes;
  const gameQuestTaskRow=`<div class="timeTaskRow timeGameQuestRow"><button type="button" class="timeTaskName" data-time-gamequest-detail="1"><span>游戏作战区</span><b>游戏作战区</b></button><div class="timeTaskMeter" style="--w:${gameQuestWeekUsed?100:0}%"><i></i></div><div class="timeTaskValue"><b>${fmtMinutes(gameQuestWeekUsed)}</b><span>今日 ${fmtMinutes(gameQuestTodayUsed)}</span></div><div class="timeTargetEdit timeTargetReadOnly"><span>统计</span><b>整体计时</b></div></div>`;
...
function renderOrbitPanel(){
  ...
  const summary=`${done}/${total}${carry?` · 遗留${carry}`:""}`;
  el.innerHTML=`<details class="orbitDrawer" ${isOrbitDrawerOpen()?"open":""}><summary><span class="orbitDrawerMini" style="--pct:${pct}%"><b>${pct}%</b></span><span class="orbitDrawerText"><strong>今日执行环</strong><em>${summary}</em></span><span class="orbitDrawerToggle">展开</span></summary><div class="orbitDrawerBody"><div class="orbitLayout"><div class="orbitCore" style="--pct:${pct}%"><div class="orbitCenter"><span class="orbitPct">${pct}%</span><span class="orbitLabel">TODAY</span><span class="orbitTiny">${summary}</span></div></div><div class="orbitSide"><div class="orbitTop"><div><div class="orbitTitle">Task Ring</div><div class="orbitSub">按日推进，异常高亮。</div></div></div><div class="orbitDays">${nodes}</div></div></div></div></details>`;
}
