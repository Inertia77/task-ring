...
function completeGameQuestDay(dayId,sourceEl=null,cycle=cycleYmd){
  const normalizedDay=Number(dayId);
  const entries=gameQuestEntriesForDay(normalizedDay,gameQuestConfig);
  const remaining=entries.reduce((sum,entry)=>sum+(entry.total-entry.done),0);
  if(!entries.length){showToast("这一天没有需要完成的游戏任务","warn");return}
  if(!remaining){showToast("这一天的游戏任务已经全部完成","ok");return}
  entries.forEach(entry=>{
    entry.tasks.forEach(task=>{
      if(!isGameQuestItemDone(entry.game.id,normalizedDay,task.id,cycle))syncSetItem(gameQuestItemKey(entry.game.id,normalizedDay,task.id,cycle),true);
    });
    syncSetItem(gameQuestDoneKey(entry.game.id,normalizedDay,cycle),true);
  });
  playCompletionEffect({level:"daily",category:"gamecreate",anchor:sourceEl,title:`${dayName(normalizedDay)}游戏日清全部完成`,eventId:`gq-day:${cycle}:d${normalizedDay}:${Date.now()}`,date:ymd(operationalNow)});
  showToast(`已完成${normalizedDay===today?"今日":"该日"}全部 ${remaining} 项游戏任务`,"ok");
  renderAll();
}
...
function gameQuestCompleteDayButtonHtml(dayId,stats=gameQuestStats(dayId)){
  const allDone=stats.total>0&&stats.done>=stats.total;
  const empty=stats.total===0;
  const label=Number(dayId)===today?"一键完成今日任务":"一键完成该日任务";
  const ariaLabel=allDone?`${dayName(dayId)}游戏任务已全部完成`:label;
  return `<button type="button" class="gameQuestCompleteDayBtn ${allDone?"done":""}" data-gq-complete-day="${Number(dayId)}" data-cycle="${escapeHtml(cycleYmd)}" ${allDone||empty?"disabled":""} aria-label="${escapeHtml(ariaLabel)}"><span aria-hidden="true">✓</span><span><b>${allDone?"游戏任务已全部完成":label}</b><em>${empty?"暂无任务":allDone?`${stats.done}/${stats.total} 已勾选`:`剩余 ${Math.max(0,stats.total-stats.done)} 项`}</em></span></button>`;
}
...
function renderGameQuestPanel(){
  ...
  if(gameQuestBoardMode==="week"){
    ...
    body=`<div class="gameQuestDailyPane">
      ...
      <div class="gameQuestSubHead"><span>${escapeHtml(dayName(gameQuestSelectedDay))}${gameQuestSelectedDay===today?"｜今日":""}</span>${gameQuestCompleteDayButtonHtml(gameQuestSelectedDay,selectedStats)}<div class="gameQuestMetricSet"><span class="gameQuestMetric"><strong>${selectedStats.done}/${selectedStats.total}</strong><em>ITEMS</em></span><span class="gameQuestMetric"><strong>${selectedStats.cardsDone}/${selectedStats.cards}</strong><em>大任务</em></span><span class="gameQuestMetric remaining"><strong>${Math.max(0,selectedStats.total-selectedStats.done)}</strong><em>剩余</em></span></div></div>
...
document.body.addEventListener("click",e=>{
  const completeDayBtn=e.target.closest("[data-gq-complete-day]");
  if(!completeDayBtn)return;
  e.preventDefault();
  e.stopImmediatePropagation();
  completeGameQuestDay(Number(completeDayBtn.dataset.gqCompleteDay),completeDayBtn,completeDayBtn.dataset.cycle||cycleYmd);
},true);
