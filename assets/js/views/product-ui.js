...
(function(){
  ...
  function dailyCard(o){
    ...
    return `<article class="dailyCard ${st.done?"done completed":""} ${st.overdue||st.failed?"overdue":""} ${st.warn?"carryover":""} ${t.important?"highPriority":""} ${running?"running":""}" style="--daily-accent:${c.color||"var(--color-line-strong)"}" data-daily-task="${escapeHtml(t.id)}">
      ...
      <div class="dailyActions">
        ...
        <button type="button" class="manualTimeTiny" data-manual-time-entry="task" data-manual-task-id="${escapeHtml(t.id)}" title="补记忘记开始的时间">+补记</button>
  ...
  window.renderWeeklyPlanPanel=function(){
    ...
    const summaries=rows.map(row=>{const def=timeCategoryDefs[row.cat]||timeCategoryDefs.life;const pct=row.target?Math.min(100,Math.round(row.used/row.target*100)):0;return `<div class="allocationChip" data-weekly-category="${escapeHtml(row.cat)}" style="--w:${pct}%"><div><b>${escapeHtml(def.short)}</b></div><em>${fmtMinutes(row.used)}${row.target?` / ${fmtMinutes(row.target)}`:""} · 完成 ${row.done}/${row.count}</em><i></i></div>`}).join("");
    const overviewKey=detailKey("weekly-overview",cycleYmd);
    const overview=activeCategory==="all"&&summaries?`<details class="weeklyOverview" data-ui-details-key="${escapeHtml(overviewKey)}"${openAttr(overviewKey,true)}><summary><div class="sectionSummary"><div class="sectionSummaryMain weeklyOverviewTitle"><b>全部任务完成概览</b><span>${rows.length} 个分类 · 投入 ${fmtMinutes(totalUsed)}${totalTarget?` / ${fmtMinutes(totalTarget)}`:""} · 完成 ${doneCount}/${tasks.length}</span></div><div class="weeklyOverviewToggle" aria-hidden="true"><span class="weeklyOverviewClosedLabel">展开</span><span class="weeklyOverviewOpenLabel">收起</span><span class="summaryChevron">⌄</span></div></div></summary><div class="weeklyOverviewBody"><div class="allocationRibbon">${summaries}</div></div></details>`:"";
  ...
  function gameMetrics(todayStats,weeklyStats,visibleStats,includeWeekly=true){
    const current=visibleStats||todayStats;
    return `<div class="gameQuestMetricSet"><span class="gameQuestMetric"><strong>${current.done}/${current.total}</strong><em>完成项目</em></span><span class="gameQuestMetric"><strong>${current.cardsDone}/${current.cards}</strong><em>完成游戏</em></span><span class="gameQuestMetric"><strong>${Math.max(0,current.total-current.done)}</strong><em>剩余</em></span>${includeWeekly?`<span class="gameQuestMetric"><strong>${weeklyStats.done}/${weeklyStats.total}</strong><em>本周指标</em></span>`:""}</div>`;
  }
  ...
  window.renderGameQuestPanel=function(){
    ...
    const top=`<div class="gameQuestTopBar taskAreaHeader"><div class="gameQuestTopTitle taskAreaHeaderCopy"><span class="taskAreaEyebrow">GAME QUEST</span><strong class="taskAreaTitle">游戏作战区</strong><em class="taskAreaDescription">今日清理与本周作战池同页展示，不再来回切换。</em></div><div class="gameQuestHeroSide taskAreaHeaderSide"><div class="gameQuestTopMeter"><span class="gameQuestMiniRing" style="--p:${todayStats.pct}%"><i>${todayStats.pct}%</i></span><span class="gameCommandCopy"><small>PROGRESS</small><b>${todayStats.done}/${todayStats.total}</b><em>所选日期</em></span></div><button type="button" class="gameCommandBtn gameQuestTodayQuick" id="gameQuestTodayBtn"><span class="gameCommandIcon">◎</span><span class="gameCommandCopy"><small>TODAY</small><b>今日</b><em>${gameQuestSelectedDay===today?"当前日期":"回到今天"}</em></span></button><button type="button" class="gameQuestTopTimer ${gqActive?"active":""}" data-timer-start-gamequest="1" data-gamequest-day="${gameQuestSelectedDay}" data-cycle="${escapeHtml(cycleYmd)}"><span class="gameCommandIcon">${gqActive?(active.paused?"Ⅱ":"◷"):"◷"}</span><span class="gameCommandCopy"><small>TIMER</small><b ${gqActive?'data-live-timer':''}>${gqActive?fmtTimer(activeTimerElapsedSeconds(active)):"开始计时"}</b><em>${gqActive?(active.paused?"已暂停":"游戏计时中"):`本周 ${fmtMinutes(taskWeekMinutesUsed("gamequest-board"))}`}</em></span></button><button type="button" class="gameCommandBtn gameQuestTopManual" data-manual-time-entry="gamequest" title="补记忘记开始的游戏时间"><span class="gameCommandIcon">＋</span><span class="gameCommandCopy"><small>MANUAL</small><b>补记</b><em>游戏时间</em></span></button><button type="button" class="gameCommandBtn gameQuestEditQuick" data-open-game-editor><span class="gameCommandIcon">✎</span><span class="gameCommandCopy"><small>QUEST</small><b>编辑任务</b><em>日常与周常</em></span></button></div></div>`;

    const dailyEntries=gameQuestEntriesForDay(gameQuestSelectedDay);
    const selected=selectedEntry(dailyEntries);
    const activeId=selected?.game?.id||"";
    const dailyCollapsed=isGameQuestCollapsed();
    const dailyLabel=`${escapeHtml(dayName(gameQuestSelectedDay))}${gameQuestSelectedDay===today?"｜今日":""}`;
    const dailyBody=`<section class="gameQuestDailyPane ${dailyCollapsed?"collapsed":""}" aria-label="今日清理"><div class="gameQuestDailyCollapseBar"><button type="button" class="gameQuestDailyToggle" data-gq-collapsed-toggle aria-expanded="${dailyCollapsed?"false":"true"}" aria-controls="gameQuestDailyBody"><span class="gameQuestDailyToggleCopy"><b>今日清理</b><em>${dailyLabel} · 完成 ${todayStats.done}/${todayStats.total} · 全周日清 ${weekStats.done}/${weekStats.total}</em></span><strong>${todayStats.pct}%</strong><span class="gameQuestDailyToggleChevron" aria-hidden="true">⌄</span></button>${gameQuestCompleteDayButtonHtml(gameQuestSelectedDay,todayStats)}</div><div class="gameQuestDailyBody" id="gameQuestDailyBody" ${dailyCollapsed?"hidden":""}><div class="gameQuestDays">${gameQuestDayTabsHtml()}</div>${gameTabs(dailyEntries,activeId)}<div class="gameQuestSubHead"><span>${dailyLabel}</span>${gameMetrics(todayStats,weeklyStats,todayStats)}</div><div class="gameQuestGrid focused">${selected?gameQuestCardHtml(selected,gameQuestSelectedDay):'<div class="gameQuestEmpty"><b>这一天没有清理任务</b><span>可以返回今日，或在游戏任务编辑器添加指定日任务。</span><button type="button" id="gameQuestTodayBtn">返回今日</button></div>'}</div></div></section>`;

    const weeklyEntries=gameQuestWeeklyEntries();
    if(gameQuestWeeklyFilter!=="all"&&!weeklyEntries.some(e=>String(e.game.id)===String(gameQuestWeeklyFilter)))gameQuestWeeklyFilter="all";
    const visible=gameQuestWeeklyFilter==="all"?weeklyEntries:weeklyEntries.filter(e=>String(e.game.id)===String(gameQuestWeeklyFilter));
    ...
    const filters=`<nav class="gameQuestFilterTabs" aria-label="本周游戏筛选"><button type="button" class="${gameQuestWeeklyFilter==="all"?"active":""}" data-gq-weekly-filter="all"><span>全部</span><b>${weeklyStats.done}/${weeklyStats.total}</b></button>${weeklyEntries.map(e=>`<button type="button" class="${gameQuestWeeklyFilter===String(e.game.id)?"active":""}" data-gq-weekly-filter="${escapeHtml(e.game.id)}"><span>${escapeHtml(e.game.short||e.game.name)}</span><b>${e.done}/${e.total}</b></button>`).join("")}</nav>`;
    const weeklyOverview=gameQuestWeeklyFilter==="all"?`<div class="gameQuestSubHead gameQuestOverallSummary"><span>全部游戏完成概览</span>${gameMetrics(actualTodayStats,weeklyStats,visibleStats,false)}</div>`:"";
    ...
    const weeklyBody=`<section class="gameQuestWeeklyPane" aria-label="本周作战池"><div class="gameQuestMetaStrip"><span><b>本周作战池</b> · 周常、深境与本周一次性目标</span><em>${weeklyStats.pct}% WEEK POOL</em></div>${filters}${weeklyOverview}<div class="gameQuestGrid focused">${cards||'<div class="gameQuestEmpty"><b>本周作战池为空</b><span>从游戏任务编辑器添加周任务。</span><button type="button" data-open-game-editor>打开编辑器</button></div>'}</div></section>`;

    panel.innerHTML=`<div class="gameQuestShell">${top}<div class="gameQuestBoardSections">${dailyBody}${weeklyBody}</div></div>`;
