...
(function(){
  ...
  window.renderGameQuestPanel=function(){
    ...
    const dailyBody=`<section class="gameQuestDailyPane" aria-label="今日清理"><div class="gameQuestMetaStrip"><span><b>今日清理</b> · 今日指标 ${actualTodayStats.done}/${actualTodayStats.total} · 全周日清 ${weekStats.done}/${weekStats.total}</span><em>${todayStats.pct}% DAILY</em></div><div class="gameQuestDays">${gameQuestDayTabsHtml()}</div>${gameTabs(dailyEntries,activeId)}<div class="gameQuestSubHead"><span>${escapeHtml(dayName(gameQuestSelectedDay))}${gameQuestSelectedDay===today?"｜今日":""}</span>${gameQuestCompleteDayButtonHtml(gameQuestSelectedDay,todayStats)}${gameMetrics(todayStats,weeklyStats,todayStats)}</div><div class="gameQuestGrid focused">${selected?gameQuestCardHtml(selected,gameQuestSelectedDay):'<div class="gameQuestEmpty"><b>这一天没有清理任务</b><span>可以返回今日，或在游戏任务编辑器添加指定日任务。</span><button type="button" id="gameQuestTodayBtn">返回今日</button></div>'}</div></section>`;
