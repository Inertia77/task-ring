...
(function(){
  ...
  window.renderTimePanel=function(){
    ...
    el.innerHTML=`<div class="timePanelShell v20"><div class="timePanelTop">${activeHtml}</div><div class="timePanelHeader"><div><span>TIME LEDGER</span><b>时间账本</b><em>总览看方向，任务页查账，明细页删错账。</em></div><div class="timeStatCards"><div><span>今日</span><b>${fmtMinutes(totalToday)}</b></div><div><span>本周</span><b>${fmtMinutes(totalWeek)}</b></div></div></div><nav class="timeSubTabs" aria-label="时间账本切换"><button type="button" class="${timeLedgerView==="overview"?"active":""}" data-time-tab="overview">总览</button><button type="button" class="${timeLedgerView==="tasks"?"active":""}" data-time-tab="tasks">任务账</button><button type="button" class="${timeLedgerView==="logs"?"active":""}" data-time-tab="logs">明细</button></nav>${body}</div>`;
  };
})();
