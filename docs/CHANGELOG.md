...
## v23 · 游戏编辑器「勾星期」改版 + 全局游戏化质感 · 2026-07-07

- **游戏作战区编辑器·每天/指定日**：不再一天天切换、复制粘贴文本。改成和普通任务编辑器同款体验——
  每个游戏一张卡，每条任务写名字 + 勾选它要出现的星期（勾满一整周＝每日任务，只勾几天＝指定日任务）。
  底层存储仍是 `schedule[星期][游戏]=[任务]`，编辑时用中间模型 `dailyByGame` 聚合/展开，保存格式不变、旧勾选不受影响。
  实现见 `app.js`：`buildGameQuestDailyByGame` / `applyDailyByGameToSchedule` / `collectGameQuestEditorState` / `renderGameQuestEditor`。
- **本周池保持原状**：周常/深渊/危局仍是「一行一条」文本框，符合它不绑定具体某天的用法。
- **全局游戏化质感**（新增覆盖层 `100-game-quest-v3.css`，`main.css` 末尾导入）：游戏卡霓虹描边 + 光晕、
  星期勾选块点亮发光（`:has()`）、分区切换 HUD 霓虹条、按钮按压回弹。刻意不用常驻 `infinite` 动画，
  尊重系统「减少动态效果」，也避免常驻元素一直动分心/费电。
- **周计划池·手机端修复**（`v21-boot.js` + `100-game-quest-v3.css`）：
  - 点靠右的类别 tab 后不再"跳回最左"——渲染后把选中的 tab 居中滚入视野（和游戏作战区同款处理）。
  - 顶部统计不再把任务压到很下面：三块 KPI 压成一行 3 列，类别配额条（allocationRibbon）从"竖堆 5~6 行"
    改成横向滑动一行；信息一条没删，手机上任务大约上移一屏。桌面端不受影响（改动只在 ≤760px 生效）。
